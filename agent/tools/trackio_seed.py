...
try:  # huggingface_hub versions before the bucket/volume API lack these.
    from huggingface_hub import Volume, create_bucket  # type: ignore[attr-defined]
except ImportError:  # pragma: no cover - depends on installed hub version
    Volume = None  # type: ignore[assignment]
    create_bucket = None  # type: ignore[assignment]
...
def _ensure_bucket_mounted(
    ...
    """
    if create_bucket is None or Volume is None or not hasattr(api, "set_space_volumes"):
        if log:
            log(
                "trackio bucket mount skipped: installed huggingface_hub lacks "
                "bucket/Space volume APIs; dashboard files will still be seeded"
            )
        return
