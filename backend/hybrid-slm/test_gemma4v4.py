"""Quick test for Gemma 4 V4 architecture (DENSE)"""
...
def test_gemma4v4():
    """Test Gemma 4 V4 model (DENSE)"""
    config = Gemma4Config()
    
    print("Testing Gemma 4 V4 Configuration (DENSE):")
    print(f"  Hidden size: {config.hidden_size}")
    print(f"  Layers: {config.num_hidden_layers}")
    print(f"  Pattern: {config.get_layer_pattern()}")
    print(f"  Sliding window: {config.sliding_window}")
    print(f"  GQA: {config.num_key_value_heads} KV heads")
    ...
    # Show layer pattern
    print(f"  Layer pattern (S=Sliding, F=Full):")
    ...
    print("\n✓ Gemma 4 V4 (DENSE) model test passed!")
